...
- **Open**:
  - **Meta-stacking / hard-gate over [rule, P_flip, P_spec, P_main]**
    can plausibly exploit the AUC-0.9 flip-detector signal that the
    hand-coded blends in gated_pipeline v1 (too soft) and v2 (too hard)
    failed to capture. Current best (LGBM+DGP 0.97271) already encodes
    the flip-detectable signal implicitly; the upside comes from
    letting a meta-model route predictions to the flipped-row specialist
    precisely where P_flip is high. Upper bound bounded by the 99.4%
    flip-direction recall × flip-detection AUC.
  - Ordinal-aware loss to cut Medium↔High confusion is still untested.
- **Neural-net models are completely untried** — no MLP, TabNet, or
  torch/keras code anywhere in the repo. The host states in
  `brief.md:74` that the synthetic labels were generated by a deep
  learning model; an MLP might interpolate the near-threshold noise
  band in a way axis-aligned trees structurally can't. Unknown upside,
...
- **Ruled out**:
  ...
  - **Hand-coded soft-blend of rule + flip-prob + specialist**
    ...
    on clean rows where P_flip > 0. A learnable meta-model is required
    (see Open bets); this exact blend mechanism is ruled out.
- **Confirmed (new)**:
