"""XGBoost on the same 43-feature set as LGBM-dist.

Goal: real model-family diversity (XGBoost's tree construction
differs from LGBM's — it is level-wise + global histogram, while
LGBM is leaf-wise + local histogram + exclusive feature bundling).
If XGBoost at OOF ~= LGBM and their errors don't fully overlap, a
blend with the 5-seed LGBM-dist bag should lift above 0.97289.

Pipeline mirrors benchmark_dist.py and seed_bag_dist.py:
  - same 5-fold stratified split (seed=42)
  - same 43-feature pack (distance-to-threshold + DGP score/rule)
  - integer-encoded categoricals wrapped in pandas Categorical
    dtype so xgb.DMatrix(enable_categorical=True) handles them
    natively
  - tune log-bias on OOF via coord-ascent
Artefacts:
  scripts/artifacts/oof_xgb_dist.npy
  scripts/artifacts/test_xgb_dist.npy
  scripts/artifacts/xgb_dist_results.json
  submissions/submission_xgb_dist_tuned.csv
"""
...
def main() -> None:
    ...
    # XGBoost native-categorical requires pandas Categorical dtype
    for c in cat_cols:
    ...
    print("\n=== XGBoost + distance-to-threshold features (OOF bal_acc) ===")
    print(f"  argmax               : {argmax_bal:.5f}")
    print(f"  prior-reweight       : {reweight_bal:.5f}")
    print(f"  tuned log-bias       : {tuned_bal:.5f}")

    np.save(ART_DIR / "oof_xgb_dist.npy", oof)
    np.save(ART_DIR / "test_xgb_dist.npy", test_pred)
    with open(ART_DIR / "xgb_dist_results.json", "w") as f:
        json.dump({
            "seed": SEED,
            "n_folds": N_FOLDS,
    ...
    pd.DataFrame({ID: te[ID], TARGET: [IDX2CLS[i] for i in tuned_test_idx]}).to_csv(
        OUT_DIR / "submission_xgb_dist_tuned.csv", index=False
    )
