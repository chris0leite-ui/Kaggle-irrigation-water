...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X_tr, y_smoke)):
        if fold >= N_FOLDS:
            break
        ck_oof = ART / f"oof_xgb_metastack_v6{suffix}_fold{fold}.npy"
        ck_test = ART / f"test_xgb_metastack_v6{suffix}_fold{fold}.npy"
        ck_meta = ART / f"oof_xgb_metastack_v6{suffix}_fold{fold}_meta.json"
        ...
        test_meta_folds.append(tp)
        # checkpoint immediately to survive rehydrate
