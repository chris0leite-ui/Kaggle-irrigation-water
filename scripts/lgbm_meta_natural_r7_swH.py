"""R7: LightGBM-meta with H-class sample upweight targeting drift_H ≈ -0.20.

R5 finding: LightGBM-meta has +0.00066 argmax accuracy lift over RF
natural, but -0.00008 tuned bal_acc because its drift_H = 0.00 (too
calibrated) vs RF natural's drift_H = -0.20 (macro-recall optimum).

R4 finding: macro-recall optimum lives at slight rare-class
over-confidence (drift_H ≈ -0.20), not at perfect natural-cal.

R7 mechanism: mild H-class sample upweight via sample_weight[y==H] = SW_H.
Trees fit H-class loss SW_H times harder → raw H probs higher → tuned
bias_H is LOWER (less correction needed) → drift_H drifts toward -0.20.

If R7 lands tuned bias_H near 3.20 (matching RF natural's optimum), the
+0.00066 argmax accuracy may compound with calibration matching to give
real Δ tuned ≥ +0.0002.

SW_H tested via env var (default 1.50; can be overridden).
Risk: too-aggressive SW_H replicates LR meta-stacker class_weight=
'balanced' failure pattern (LB regress). 1.50 is mild vs ~17x balanced.

Outputs (suffix _r7_swH150):
  oof_lgbm_meta_natural_r7_swH150.npy
  test_lgbm_meta_natural_r7_swH150.npy
  lgbm_meta_natural_r7_swH150_results.json
  submission_lgbm_meta_natural_r7_swH150_standalone.csv
"""
...
SW_H = float(os.environ.get("SW_H", "1.5"))
SUFFIX = f"_r7_swH{int(SW_H * 100):03d}"
...
def main():
    ...
    log(f"config: n_folds={n_folds} n_estimators={n_est} SW_H={SW_H}")
    log(f"  lgb_params={lgb_params}")

    # Per-row sample weight: H-class upweighted to push raw H probs higher
    sw_full = np.ones(len(y_use), dtype=np.float32)
    sw_full[y_use == 2] = SW_H  # class index 2 = High
    log(f"  sample_weight: L=1.0, M=1.0, H={SW_H}  (H rows: {(y_use == 2).sum():,})")
    ...
    for fold, (tr_idx, va_idx) in enumerate(splits, 1):
        t0 = time.time()
        train_set = lgb.Dataset(X_tr_s[tr_idx], y_use[tr_idx], weight=sw_full[tr_idx])
    ...
    np.save(ART / f"oof_lgbm_meta_natural{suffix}.npy", oof)
    np.save(ART / f"test_lgbm_meta_natural{suffix}.npy", test_pred)
    ...
    out_p = ART / f"lgbm_meta_natural{suffix}_results.json"
    ...
    out_csv = SUB / f"submission_lgbm_meta_natural{SUFFIX}_standalone.csv"
    sub = pd.DataFrame({
        "id": test_ids,
    ...
    out_d = ART / f"lgbm_meta_natural{SUFFIX}_diag.json"
