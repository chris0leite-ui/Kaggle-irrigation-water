"""Seed-7 variant of xgb_dist_routed_v3.py for bagging variance reduction.

IDENTICAL to xgb_dist_routed_v3.py EXCEPT:
  - XGB's internal RNG uses XGB_SEED=7 instead of 42.
  - StratifiedKFold still uses random_state=42 (fold split pinned so
    OOFs align with every other on-disk artefact).
  - Output filenames suffixed `_seed7`.

Used as the second leg of a 2-seed bag (averaged with seed=42's
oof_xgb_dist_routed_v3.npy downstream).

Artefacts:
  scripts/artifacts/oof_xgb_dist_routed_v3_seed7.npy
  scripts/artifacts/test_xgb_dist_routed_v3_seed7.npy
  scripts/artifacts/xgb_dist_routed_v3_seed7_results.json
  submissions/submission_xgb_dist_routed_v3_seed7_tuned.csv
"""
...
FOLD_SEED = 42       # fold split PINNED at 42 so OOFs align with seed=42 artefacts
XGB_SEED = 7         # only XGB's internal RNG varies
N_FOLDS = 5
ROUTED_SCORES = (0, 1, 2)
...
def main() -> None:
    log(f"loading data  (FOLD_SEED={FOLD_SEED} pinned, XGB_SEED={XGB_SEED})")
    ...
    log(f"running 5-fold stratified XGBoost with score-{{0,1,2}} routing (XGB_SEED={XGB_SEED})")
    # NB: fold split still uses random_state=42 so OOF rows align with other artefacts
    ...
    rule_pred_on_routed = np.zeros(tr_routed_mask.sum(), dtype=np.int32)
    ...
    print("\n=== XGBoost-dist + score-{0,1,2} routing SEED7 (OOF bal_acc) ===")
    print(f"  XGB_SEED                     : {XGB_SEED}  (fold split seed={FOLD_SEED})")
    ...
    print(f"  baseline seed=42 routed_v3   : 0.97332 (on-disk reference)")
    print(f"  Δ vs seed=42                 : {tuned_bal - 0.97332:+.5f}")

    np.save(ART_DIR / "oof_xgb_dist_routed_v3_seed7.npy", oof)
    np.save(ART_DIR / "test_xgb_dist_routed_v3_seed7.npy", test_pred)
    with open(ART_DIR / "xgb_dist_routed_v3_seed7_results.json", "w") as f:
        json.dump({
            "fold_seed": FOLD_SEED,
            "xgb_seed": XGB_SEED,
            "n_folds": N_FOLDS,
    ...
    pd.DataFrame({ID: te[ID], TARGET: [IDX2CLS[i] for i in tuned_test_idx]}).to_csv(
        OUT_DIR / "submission_xgb_dist_routed_v3_seed7_tuned.csv", index=False
    )
