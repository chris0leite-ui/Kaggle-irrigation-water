"""LGBM-skte UNIFORM-TE variant — sklearn TE on cats + binned-numerics.

Tests the structural hypothesis from 2026-04-29 LGBM-skte production
diagnostic: rawashishsin v3's natural calibration (bias drift 0.000 on
High at production scale) likely comes from applying sklearn TE to
EVERY feature, not just cat-tuples. Our V10 recipe applies TE to 117
cat-tuples and keeps 92 numerics ungrouped, which broke calibration at
production (drift 1.000).

This variant:
  - Bins each raw numeric to 100 quantile bins (qcut, duplicates='drop')
  - Adds binned numerics + threshold flags to te_cols
  - Keeps freq + orig_stats + logits as raw numerics
  - Result: ~128 cat-tuples × 3 classes = ~384 sklearn-TE features
    (vs 117×3 = 351 in the original LGBM-skte)
  - Same LGBM HPs (num_leaves=8, lr=0.05, max_bin=1100, no reg,
    ORIG_ROW_WEIGHT=0.5)

If natural-cal returns at production (drift ≤ 0.3), it validates the
"uniform TE" hypothesis and produces a strong 11th candidate for v3
RF rebuild. If drift remains >0.7, it points to FE breadth itself
(143 cols vs rawashishsin's 133) as the binding constraint, not the
TE-on-everything mechanism.

Wall budget: ~30-60 min CPU (similar to recipe_lgbm_skte.py base).
Per-fold checkpoints for rehydrate resilience.
"""
...
ORIG_ROW_WEIGHT = 0.5
N_QBINS = 100  # quantile bins per raw numeric
...
SUFFIX = "_lgbm_skte_full" + ("_smoke" if SMOKE else "")
FINAL_OOF_NAME = ("recipe_full_te_lgbm_skte_full"
...
def add_qbinned_numerics(train, test, orig, nums, n_bins=N_QBINS):
    """Bin each raw numeric to n_bins quantile bins (combined train+test+orig).
    Returns the list of new column names; also adds them to each df.
    """
    new_cols = []
    for c in nums:
        combined = pd.concat([train[c], test[c], orig[c]], ignore_index=True).astype(float)
        binned, _ = pd.qcut(combined, q=n_bins, labels=False, duplicates='drop', retbins=True)
        s = len(train); t = s + len(test)
        col = f"{c}_qbin"
        train[col] = binned[:s].astype(np.int16)
        test[col] = binned[s:t].astype(np.int16)
        orig[col] = binned[t:].astype(np.int16)
        new_cols.append(col)
    return new_cols
...
def load_and_engineer():
    ...
    log("uniform-TE additions: q-binned raw numerics")
    nums_qbin = add_qbinned_numerics(train, test, orig, nums)
    log(f"  nums_qbin: {len(nums_qbin)} cols × {N_QBINS} bins each")
    ...
    info = dict(
        ...
        orig_stats=orig_stats_cols, nums_qbin=nums_qbin,
        # KEY DIFFERENCE: te_cols now includes binned numerics AND threshold flags
        te_cols=cats + combos + digits + num_as_cat + tres + nums_qbin,
    )
    log(f"  feature groups: cats={len(cats)} combos={len(combos)} "
        ...
        f"orig_stats={len(orig_stats_cols)} nums_qbin={len(nums_qbin)} "
        f"te_cols={len(info['te_cols'])} (was 117 in base, +nums_qbin)")
...
def run_one_fold(fold, tr_idx, va_idx, train, test, orig, info, y, y_orig):
    ...
    # Numeric feats KEPT raw (not passed through TE): logits, freq, orig_stats.
    # Note: tres + nums_qbin are NOW in te_cols, so removed from numeric_feats.
    numeric_feats = (info["logits"] + info["freq"] + info["orig_stats"])

    log(f"  fitting sklearn TargetEncoder(multiclass, cv=5) on "
        f"{len(te_cols)} input cols (cats + combos + digits + num_as_cat "
        f"+ tres + nums_qbin)")
    t0 = time.time()
    ...
    X_tr_num = X_tr[numeric_feats].to_numpy(dtype=np.float32) if numeric_feats else np.zeros((len(X_tr), 0), dtype=np.float32)
    X_va_num = X_va[numeric_feats].to_numpy(dtype=np.float32) if numeric_feats else np.zeros((len(X_va), 0), dtype=np.float32)
    X_te_num = X_te[numeric_feats].to_numpy(dtype=np.float32) if numeric_feats else np.zeros((len(X_te), 0), dtype=np.float32)
    X_or_num = X_or[numeric_feats].to_numpy(dtype=np.float32) if numeric_feats else np.zeros((len(X_or), 0), dtype=np.float32)
    ...
    log(f"  combined train: {len(X_combined)} rows  "
        ...
        f"orig sw_mult={ORIG_ROW_WEIGHT}  total features={X_tr_full.shape[1]}")

    n_est = 300 if SMOKE else 2600
    params = dict(
        objective="multiclass", num_class=3,
        n_estimators=n_est, learning_rate=0.05,
        num_leaves=8, max_depth=-1,
        bagging_fraction=0.9, bagging_freq=5,
        feature_fraction=0.8, max_bin=1100,
        reg_alpha=0.0, reg_lambda=0.0,
        n_jobs=-1, random_state=SEED + fold,
        verbosity=-1,
    )

    log(f"  training LGBM (num_leaves=8, lr=0.05, max_bin=1100) on "
...
def main():
    ...
    summary = dict(
        seed=SEED, n_folds=N_FOLDS, n_qbins=N_QBINS,
        ...
        model="LightGBM num_leaves=8, lr=0.05, max_bin=1100, no reg (rawashishsin parity)",
        te_input="cats + combos + digits + num_as_cat + tres + nums_qbin (UNIFORM)",
    )
