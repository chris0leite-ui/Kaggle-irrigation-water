"""Option 1 family-diversity addition: LightGBM natural-cal rebuild on V10
recipe FE with sklearn TargetEncoder(multiclass, cv=5).

Mirror of recipe_catboost_skte.py (Pick 2b) but swaps CatBoost -> LightGBM.
Tests whether the rawashishsin natural-calibration mechanism (sklearn TE
internal CV-shuffled smoothing replacing L1/L2 reg) generalises across
gradient-boosted tree families.

Knobs match natural-cal regime:
  num_leaves=8       (= depth ~ 3)
  learning_rate=0.05
  reg_alpha=0        (NO reg — sklearn TE replaces it)
  reg_lambda=0       (NO reg)
  n_estimators=2600  (rawashishsin parity)
  bagging_fraction=0.8 + bagging_freq=5
  feature_fraction=0.8
  min_data_in_leaf=2
  ORIG_ROW_WEIGHT=0.5  (concat orig at half weight)
  no class_weight     (natural calibration via sklearn TE smoothing)

Diagnostic gate: bias_H ∈ [-0.5, +1.0] after tune_log_bias ⇒ natural cal.
Wall budget: ~50 min CPU (5 folds × ~10 min).
"""
...
FINAL_OOF_NAME = ("recipe_full_te_lgbm_skte"
...
def run_one_fold(fold, tr_idx, va_idx, train, test, orig, info, y, y_orig):
    ...
    log(f"  fitting sklearn TargetEncoder(multiclass, cv=5, smooth=auto) "
        f"on {len(te_cols)} cat-tuples")
    t0 = time.time()
    ...
    lgb_params = dict(
        ...
        num_leaves=8,                # depth ~ 3
        learning_rate=0.05,
        n_estimators=300 if SMOKE else 2600,
        feature_fraction=0.8,
        bagging_fraction=0.8,
        bagging_freq=5,
        min_data_in_leaf=2,
        reg_alpha=0.0,                # natural-cal: NO L1
        reg_lambda=0.0,               # natural-cal: NO L2
        random_state=SEED + fold,
        n_jobs=-1,
        verbose=-1,
    )

    log(f"  training LightGBM (num_leaves=8, no reg, lr=0.05) on "
    ...
    model.fit(
        ...
        callbacks=[
            lgb.early_stopping(stopping_rounds=50 if SMOKE else 200, verbose=False),
...
def main():
    ...
    if RUN_FOLD is not None:
        log(f"finished single fold {RUN_FOLD}; rerun without RUN_FOLD to "
            f"aggregate when all {N_FOLDS} fold checkpoints exist")
        return
    ...
    summary = dict(
        ...
        params={k: v for k, v in dict(
            num_leaves=8, learning_rate=0.05, reg_alpha=0.0,
            reg_lambda=0.0, n_estimators=300 if SMOKE else 2600,
