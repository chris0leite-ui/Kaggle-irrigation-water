"""LGBM clone of rawashishsin v3 on V10 recipe FE — companion to A1 (XGB).

Mirrors A1 (`recipe_xgb_skte.py` on main) but swaps XGBClassifier →
LGBMClassifier with HPs translated to LGBM equivalents:
  XGB                          ↔  LGBM
  ---------------------------     ---------------------------------
  ...
  tree_method='hist'           ↔  (LGBM is histogram-based by default)

Same FE pipeline (V10 recipe + sklearn TargetEncoder cv=5 on 117
cat-tuples), same per-fold StratifiedKFold(seed=42), same
ORIG_ROW_WEIGHT=0.5, same class-balanced sample weights.

Hypothesis: rawashishsin's natural-cal property comes from the HP
regime + sklearn TE smoothing, not from the model class. LGBM
(leaf-wise growth) should produce a bias drift profile similar to
XGB (level-wise growth) under the same regime. If it does, LGBM
becomes the 11th naturally-calibrated component for v3 RF natural
meta-stacker rebuild after main's v2 (10 components) lands.

Independent of main's running A1 chains — different artifact namespace
(_lgbm_skte vs _xgb_skte / _catboost_skte). My branch produces this
ADDITIVELY; v3 RF rebuild can absorb it after A1 lands.

Per-fold checkpointing for rehydrate resilience. RUN_FOLD env var
for sequential launches if needed.

Wall budget: ~30-40 min CPU on 16-core (LGBM typically 1.5-2× faster
than XGB at depth=3 + max_bin=1100).
"""
...
SUFFIX = "_lgbm_skte" + ("_smoke" if SMOKE else "")
FINAL_OOF_NAME = ("recipe_full_te_lgbm_skte" + ("_smoke" if SMOKE else ""))
...
def run_one_fold(fold, tr_idx, va_idx, train, test, orig, info, y, y_orig):
    ...
    # rawashishsin's HPs translated to LGBM
