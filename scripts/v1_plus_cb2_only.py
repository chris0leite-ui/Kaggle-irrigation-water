...
Outputs:
  scripts/artifacts/oof_xgb_metastack_v1_plus_cb2.npy
  scripts/artifacts/test_xgb_metastack_v1_plus_cb2.npy
  scripts/artifacts/v1_plus_cb2_meta_results.json
"""
...
NEW_ADDITIONS = ["recipe_catboost_v2"]
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X_tr, y)):
        ckpt_oof  = ART / f"_v1pcb_fold{fold}_oof.npy"
        ckpt_test = ART / f"_v1pcb_fold{fold}_test.npy"
        ckpt_meta = ART / f"_v1pcb_fold{fold}_meta.json"
    ...
    np.save(ART / "oof_xgb_metastack_v1_plus_cb2.npy", oof_meta)
    np.save(ART / "test_xgb_metastack_v1_plus_cb2.npy", test_meta)
    ...
    log(f"\n  primary OOF (4-stack + v1+cb2_iso @α=0.30) = {bal(prim_o, y):.5f}")
    ...
    json_path = ART / "v1_plus_cb2_meta_results.json"
