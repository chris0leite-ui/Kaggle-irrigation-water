"""Seed-7 variant of xgb_specialist_678.py for bagging variance reduction.

IDENTICAL to xgb_specialist_678.py EXCEPT:
...
oof_xgb_spec_678.npy downstream).

Artefacts:
    scripts/artifacts/oof_xgb_spec_678_seed7.npy        (630k × 3)
    scripts/artifacts/test_xgb_spec_678_seed7.npy       (270k × 3)
    scripts/artifacts/xgb_spec_678_seed7_results.json
"""
...
def main() -> None:
    ...
    log(f"running 5-fold stratified XGB specialist on scores {{6,7,8}} (XGB_SEED={XGB_SEED})")
    # NB: stratify on overall y with random_state=FOLD_SEED=42 (pinned)
    ...
    print("\n=== XGB specialist on scores {6,7,8} SEED7 (evaluated on spec domain only) ===")
    ...
    np.save(ART_DIR / "oof_xgb_spec_678_seed7.npy", oof_spec)
    np.save(ART_DIR / "test_xgb_spec_678_seed7.npy", test_spec)
    with open(ART_DIR / "xgb_spec_678_seed7_results.json", "w") as f:
        json.dump({
            "fold_seed": FOLD_SEED,
            "xgb_seed": XGB_SEED,
            "n_folds": N_FOLDS,
