...
Pipeline:
  ...
  - Save OOF + test probs so downstream blend/gate scripts can
    route scores {6,7,8} to these probs

Artefacts:
    scripts/artifacts/oof_xgb_spec_678.npy        (630k × 3)
    scripts/artifacts/test_xgb_spec_678.npy       (270k × 3)
    scripts/artifacts/xgb_spec_678_results.json
"""
...
def main() -> None:
    ...
    log("running 5-fold stratified XGB specialist on scores {6,7,8}")
    ...
    print("\n=== XGB specialist on scores {6,7,8} (evaluated on spec domain only) ===")
    ...
    np.save(ART_DIR / "oof_xgb_spec_678.npy", oof_spec)
    np.save(ART_DIR / "test_xgb_spec_678.npy", test_spec)
    with open(ART_DIR / "xgb_spec_678_results.json", "w") as f:
        json.dump({
            "seed": SEED,
            "n_folds": N_FOLDS,
