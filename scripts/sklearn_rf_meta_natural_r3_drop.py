"""R3: RF natural meta DROP largest-drift component (recipe_full_te_catboost).

LB-best (LB 0.98129) bank has 7 components with mixed drift profiles:
  rawashishsin_2600                 max|drift| = 1.10
  recipe_full_te_catboost_natural   max|drift| = 0.80
  recipe_full_te_catboost           max|drift| = 1.20  ← LARGEST drift
  recipe_full_te                    max|drift| = 0.90
  realmlp                           max|drift| = 0.70
  xgb_corn                          max|drift| = 0.10  ← only strict pass
  xgb_dist_digits                   max|drift| = 0.80

R3 drops recipe_full_te_catboost (worst drift) → 6-component bank.
Tests whether reducing extreme-drift inputs HELPS or HURTS the meta's
natural-cal output (LB 0.98129 had bias_H drift -0.20).

R3 LIFTS  → fewer extreme-drift inputs improve meta calibration.
R3 NULLS  → drift composition was already at sweet spot.
R3 REGRSS → recipe_full_te_catboost was contributing real signal
            despite high drift (drift ≠ harm).

Architecture frozen identical to LB 0.98129.

Outputs (suffix _r3_drop):
  scripts/artifacts/oof_sklearn_rf_meta_natural_r3_drop.npy
  scripts/artifacts/test_sklearn_rf_meta_natural_r3_drop.npy
  scripts/artifacts/sklearn_rf_meta_natural_r3_drop_results.json
  submissions/submission_sklearn_rf_meta_natural_r3_drop_standalone.csv
"""
...
# 6-component bank: 7 LB-best DROP recipe_full_te_catboost (max|drift|=1.20).
BANK = [
    "rawashishsin_2600",                  # max|drift|=1.10
    "recipe_full_te_catboost_natural",    # max|drift|=0.80
    # "recipe_full_te_catboost",           # R3: DROPPED (max|drift|=1.20, worst)
    "recipe_full_te",                     # max|drift|=0.90
    "realmlp",                            # max|drift|=0.70
    "xgb_corn",                           # max|drift|=0.10 (only strict pass)
    "xgb_dist_digits",                    # max|drift|=0.80
...
def load_bank(n_tr, n_te):
    log(f"loading R3 6-component drop bank (drop recipe_full_te_catboost)")
    pool = {}
    for name in BANK:
