"""XGBoost-dist with rule-routing for rows at dgp_score in {1, 2}.

At train time: drop rows with dgp_score in {1, 2} from both train and
...
Artefacts:
  scripts/artifacts/oof_xgb_dist_routed.npy        (full-row OOF)
  scripts/artifacts/test_xgb_dist_routed.npy       (full-row test)
  scripts/artifacts/xgb_dist_routed_results.json
  submissions/submission_xgb_dist_routed_tuned.csv
"""
...
def main() -> None:
    ...
    log("running 5-fold stratified XGBoost with score-{1,2} routing")
    ...
    print("\n=== XGBoost-dist + score-{1,2} routing (OOF bal_acc) ===")
    ...
    np.save(ART_DIR / "oof_xgb_dist_routed.npy", oof)
    np.save(ART_DIR / "test_xgb_dist_routed.npy", test_pred)
    with open(ART_DIR / "xgb_dist_routed_results.json", "w") as f:
        json.dump({
            "seed": SEED,
            "n_folds": N_FOLDS,
    ...
    pd.DataFrame({ID: te[ID], TARGET: [IDX2CLS[i] for i in tuned_test_idx]}).to_csv(
        OUT_DIR / "submission_xgb_dist_routed_tuned.csv", index=False
    )
