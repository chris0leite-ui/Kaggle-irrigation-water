...
def main():
    log(f"=== ET natural v1 — N_EST={N_EST} FOLD_SEED={FOLD_SEED} RF_SEED={RF_SEED} ===")
    ...
    log(f"ET: n_est={n_est} max_depth={max_depth} class_weight=None")
    ...
    if not SMOKE:
        np.save(ART / f"oof_et_natural_v1{SUFFIX}.npy", oof)
        np.save(ART / f"test_et_natural_v1{SUFFIX}.npy", test_pred)
        ...
        sub_path = SUB / f"submission_et_natural_v1{SUFFIX}_standalone.csv"
    ...
    json_p = ART / f"et_natural_v1{SUFFIX}_results.json"
