...
SMOKE = os.environ.get("SMOKE", "0") == "1"  # local default: 0 (production)
...
def load_and_engineer():
    ...
    for p in sorted(KAGGLE_INPUT.rglob("*.csv")):
        log(f"  {p}")
    log("loading train / test / orig (local paths)")
    train = pd.read_csv("data/train.csv")
    test = pd.read_csv("data/test.csv")
    orig = pd.read_csv("data/archive.zip")  # original 10k dataset
