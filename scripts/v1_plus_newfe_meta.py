"""Retrain v1's xgb_metastack with 4 new recipe variants added to the pool.

The 4 additions have feature views NOT in v1's 62-component pool but
ARE trained with SKF(seed=42) so they share v1's fold-alignment
structure (which the GroupKFold + clean-pool diagnostics showed is
load-bearing for LB transfer):

  recipe_full_te_3way      — 3-way OTE on cat triples (novel)
  recipe_full_te_nndist    — k-NN distance features to 10k original
  recipe_full_te_fexw8     — W8 feature block (untested as bank component)
  recipe_full_te_instab    — instability/perturbation features

Hypothesis: each component carries signal v1 doesn't have AND is
fold-aligned, so the meta can absorb the new signal through the same
mechanism that v1 already uses.

Same XGB HPs as v1 (depth=4, reg_alpha=5, reg_lambda=5, lr=0.05).
Same 5-fold StratifiedKFold(seed=42). 66-component pool (62 + 4 new).

Outputs:
  scripts/artifacts/oof_xgb_metastack_v1_plus_newfe.npy
  scripts/artifacts/test_xgb_metastack_v1_plus_newfe.npy
...
# v1's exact 62 components + 4 new ones
V1_FULL_POOL = [
...
NEW_ADDITIONS = [
    "recipe_full_te_3way",
    "recipe_full_te_nndist",
    "recipe_full_te_fexw8",
    "recipe_full_te_instab",
]
V1_PLUS_POOL = V1_FULL_POOL + NEW_ADDITIONS


def log(m): print(f"[{time.strftime('%H:%M:%S')}] PNF: {m}", flush=True)
...
def main():
    ...
    log(f"v1 + new FE pool: {len(V1_PLUS_POOL)} components ({len(V1_FULL_POOL)} + {len(NEW_ADDITIONS)} new)")
    ...
    # Build LB-best 4-stack base (matches v1 anchor exactly)
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X_tr, y)):
        ckpt_oof  = ART / f"_v1pn_fold{fold}_oof.npy"
        ckpt_test = ART / f"_v1pn_fold{fold}_test.npy"
        ckpt_meta = ART / f"_v1pn_fold{fold}_meta.json"
    ...
    np.save(ART / "oof_xgb_metastack_v1_plus_newfe.npy", oof_meta)
    np.save(ART / "test_xgb_metastack_v1_plus_newfe.npy", test_meta)

    meta_argmax = balanced_accuracy_score(y, oof_meta.argmax(1))
    meta_tuned = bal(oof_meta, y)
    log(f"\n=== v1+newFE META standalone ===")
    ...
    log(f"\n  primary (4-stack + v1+newFE_iso @α=0.30) OOF = {bal(prim_o, y):.5f}")
    log(f"  reference: v1 primary OOF = 0.98084 (LB 0.98094, gap -0.00010)")

    # Test-side comparison vs current PRIMARY
    ...
    primary_oof = log_blend([base_o, iso_full(np.load(ART / "oof_xgb_metastack.npy"),
                                              np.load(ART / "test_xgb_metastack.npy"))[0]],
                            np.array([0.70, 0.30]))
    pred_p = (np.log(np.clip(primary_oof, 1e-12, 1)) + BIAS).argmax(1)
    pred_b = (np.log(np.clip(prim_o, 1e-12, 1)) + BIAS).argmax(1)
    pcr_p = np.array([(pred_p[y == k] == k).mean() for k in range(3)])
    pcr_b = np.array([(pred_b[y == k] == k).mean() for k in range(3)])
    pcr_d = pcr_b - pcr_p
    log(f"  PCR delta vs v1 primary: L={pcr_d[0]:+.5f} M={pcr_d[1]:+.5f} H={pcr_d[2]:+.5f}")

    # Net High flip count (G4)
    ...
    json_path = ART / "v1_plus_newfe_meta_results.json"
