"""R10: RF natural with Tier 1b 4-stack as 8th bank component.

Genuinely untried: ADD the OTHER LB-validated submission as a bank
component (not as a blend leg). LB-validated submissions on disk:
  rawashishsin_2600 (LB 0.98109) — already in bank
  sklearn_rf_meta_natural (LB 0.98129) — IS the meta we're training
  tier1b_greedy_meta (LB 0.98094) — NOT in any bank
  3-way recipe (LB 0.98005) — NOT in any bank

R10 reconstructs Tier 1b 4-stack OOF (lb3 + xgb_metastack_iso α=0.30),
adds as 8th bank component, retrains RF natural with same HPs.

Distinct from R1/R2/R3/R4:
  R1-R4 added/swapped/dropped components with high drift (cb_skte
  drift |1.50|, recipe_full_te_catboost drift |1.20|, etc.)
  R10 adds the 4-stack which has drift [+0.50, +0.40, -0.10] —
  drift_H = -0.10 (correct direction), drift_L/M moderate.

Outputs:
  oof_sklearn_rf_meta_natural_r10_with_tier1b.npy
  test_sklearn_rf_meta_natural_r10_with_tier1b.npy
  sklearn_rf_meta_natural_r10_with_tier1b_results.json
  submissions/submission_sklearn_rf_meta_natural_r10_with_tier1b_standalone.csv
"""
...
SUFFIX = "_r10_with_tier1b"

# 7 LB-best + tier1b 4-stack as 8th
BANK_BASE = [
    "rawashishsin_2600",
...
def main():
    ...
    # Build 8th component: Tier 1b 4-stack (LB 0.98094)
    ...
    log(f"  + tier1b_4stack (LB 0.98094 reconstructed)")
    ...
    log(f"=== R10 OOF argmax = {overall:.5f}  tuned = {tuned:.5f}  bias = {bias.round(4).tolist()}")
    ...
    lb_oof = np.load(ART / "oof_sklearn_rf_meta_natural_v1_lb98129.npy")
    lb_test = np.load(ART / "test_sklearn_rf_meta_natural_v1_lb98129.npy")
    lb_oof = lb_oof.astype(np.float32)
    lb_test = lb_test.astype(np.float32)
    ...
    test_pred_r10 = (safelog(test_pred) + bias).argmax(1)
    test_pred_lb = (safelog(lb_test) + bias_lb).argmax(1)
    rows_diff = (test_pred_r10 != test_pred_lb).sum()
    net_h = int(((test_pred_r10 == 2) & (test_pred_lb != 2)).sum() -
                ((test_pred_lb == 2) & (test_pred_r10 != 2)).sum())
    churn_h = int(((test_pred_r10 == 2) ^ (test_pred_lb == 2)).sum())
    ...
    sub = pd.DataFrame({"id": test_ids, TARGET: [IDX2CLS[i] for i in test_pred_r10]})
