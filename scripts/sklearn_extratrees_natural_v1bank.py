"""Tier B1: sklearn ExtraTreesClassifier meta on v1 natural-cal bank.

Mirror of sklearn_rf_meta_natural.py with two diffs:
  - ExtraTreesClassifier instead of RandomForest (bootstrap=False +
    random feature thresholds = different bagging variance structure)
  - 7-component v1 bank (LB-validated, produced LB 0.98129)
    NOT the 11-component v2 bank (which LB-regressed)

Hypothesis: ExtraTrees' randomization across feature thresholds may
produce orthogonal errors vs RF's bootstrap aggregation, even on the
same bank. If ExtraTrees standalone OOF beats v1's 0.98063 AND has
similar/better calibration, blend with v1 may compound.

Outputs:
  scripts/artifacts/oof_sklearn_extratrees_natural_v1bank.npy
  scripts/artifacts/test_sklearn_extratrees_natural_v1bank.npy
  scripts/artifacts/sklearn_extratrees_natural_v1bank_results.json
"""
...
def main():
    ...
    et_params = dict(
        ...
        bootstrap=False,                  # ExtraTrees default: no bootstrap
        n_jobs=-1, random_state=SEED,
        class_weight=None,                # natural-cal: no class upweight
        verbose=0,
    )
    log(f"ExtraTrees (natural): n_est={n_est} max_depth={max_depth} "
        f"bootstrap=False class_weight=None")
    ...
    np.save(ART / "oof_sklearn_extratrees_natural_v1bank.npy", oof)
    np.save(ART / "test_sklearn_extratrees_natural_v1bank.npy", test_pred)
    ...
    with open(ART / "sklearn_extratrees_natural_v1bank_results.json", "w") as f:
        json.dump(summary, f, indent=2)
    log(f"wrote {ART}/sklearn_extratrees_natural_v1bank_results.json")

    # Build standalone submission
    eps = 1e-9
    ...
    sub_path = SUB / "submission_sklearn_extratrees_natural_v1bank_standalone.csv"
