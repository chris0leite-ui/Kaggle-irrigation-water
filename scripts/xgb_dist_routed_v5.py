"""XGBoost-dist with rule-routing for rows at dgp_score in {0, 1, 2, 5, 9}.

At train time: drop rows with dgp_score in {0, 1, 2, 5, 9} from both train and
...
Artefacts:
  scripts/artifacts/oof_xgb_dist_routed_v5.npy        (full-row OOF)
  scripts/artifacts/test_xgb_dist_routed_v5.npy       (full-row test)
  scripts/artifacts/xgb_dist_routed_v5_results.json
  submissions/submission_xgb_dist_routed_v5_tuned.csv
"""
...
def main() -> None:
    ...
    # Per-score rule onehot probs: 0/1/2 -> Low, 5 -> Medium, 9 -> High.
    def rule_prob_for(sc_arr: np.ndarray) -> np.ndarray:
        p = np.full((len(sc_arr), 3), 1e-9, dtype=np.float64)
        p[np.isin(sc_arr, [0, 1, 2]), 0] = 1.0 - 2e-9
        p[sc_arr == 5, 1] = 1.0 - 2e-9
        p[sc_arr == 9, 2] = 1.0 - 2e-9
        return p

    log("running 5-fold stratified XGBoost with score-{0,1,2,5,9} routing")
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y)):
        ...
        # route: score in {0,1,2,5,9} -> per-score rule; else -> XGB
        va_mask = tr_routed_mask[va_idx]
        oof[va_idx[~va_mask]] = val_pred_all[~va_mask]
        oof[va_idx[va_mask]] = rule_prob_for(tr_scores[va_idx[va_mask]])
    ...
    test_pred[te_routed_mask] = rule_prob_for(te_scores[te_routed_mask])
    ...
    # diagnostic: rule accuracy per routed score
    for s in ROUTED_SCORES:
        ...
        if s in (0, 1, 2):
            ...
            rule_label = 2  # High
        hit = (y[mask] == rule_label).mean()
        log(f"  score={s}: n={mask.sum()}  rule_raw_acc={hit:.5f}  "
            f"(predicts {CLASSES[rule_label]})")
    ...
    print("\n=== XGBoost-dist + score-{0,1,2,5,9} routing (OOF bal_acc) ===")
    ...
    np.save(ART_DIR / "oof_xgb_dist_routed_v5.npy", oof)
    np.save(ART_DIR / "test_xgb_dist_routed_v5.npy", test_pred)
    with open(ART_DIR / "xgb_dist_routed_v5_results.json", "w") as f:
        json.dump({
            "seed": SEED,
            "n_folds": N_FOLDS,
    ...
    pd.DataFrame({ID: te[ID], TARGET: [IDX2CLS[i] for i in tuned_test_idx]}).to_csv(
        OUT_DIR / "submission_xgb_dist_routed_v5_tuned.csv", index=False
    )
