"""Blend-gate for the tree-leaf OTE meta-stacker (J1).

Loads oof_leaf_ote_meta.npy + test_leaf_ote_meta.npy, evaluates against:
  - LB-best 3-stack (build_lbbest_stack)
  - LB-best 4-stack = 3-stack + xgb_metastack__iso α=0.30
  - recipe_full_te (weakest anchor)

Reports Jaccard, error count, per-class recall, and fixed-bias α-sweep.
Auto-emits submission iff:
  * Δ ≥ +2e-4 OOF over LB-best 4-stack
  * per-class recall ≥ 4-stack − 5e-4 on every class
  * Jaccard < 0.97 vs LB-best 4-stack
"""
...
def main() -> None:
    ...
    oof_meta = np.load(ART / "oof_leaf_ote_meta.npy").astype(np.float32)
    test_meta = np.load(ART / "test_leaf_ote_meta.npy").astype(np.float32)
    print(f"loaded leaf-OTE meta: oof={oof_meta.shape} test={test_meta.shape}")

    # Build the LB-best 3-stack and the LB-best 4-stack (= 3-stack + meta_iso α=0.30).
    s3_o, s3_t = build_lbbest_stack(y)
    print(f"LB-best 3-stack tuned on full OOF: argmax = "
          f"{balanced_accuracy_score(y, fixed_bias_argmax(s3_o, LB_BIAS)):.5f}")
    ...
    # Standalone diagnostic on leaf-OTE meta.
    ...
    print(f"leaf-OTE standalone @ own bias: tuned={own_tuned:.5f}  bias={own_bias.tolist()}")
    print(f"leaf-OTE standalone @ LB bias:  argmax_bal={fb_argmax_bal:.5f}  errs={fb_errs}  "
          f"recL={fb_pcr[0]:.4f} recM={fb_pcr[1]:.4f} recH={fb_pcr[2]:.4f}")
    j_vs_4 = err_jaccard(fb_pred, s4_pred, y)
    print(f"Jaccard(leaf-OTE, LB-best 4-stack) @ LB bias = {j_vs_4:.4f}")

    # Iso-cal'd version of the meta (parallel to xgb_metastack__iso treatment).
    meta_iso_o, meta_iso_t = iso_cal(normed(oof_meta), normed(test_meta), y)

    # Fixed-bias α sweep vs the LB-best 4-stack.
    print("\n=== α-sweep: LB-best 4-stack × leaf-OTE-meta_iso (fixed bias) ===")
    ...
    if best["emit"]:
        ...
        out_path = SUB / f"submission_leaf_ote_meta_a{int(best['alpha']*1000):03d}.csv"
    ...
    # Strip non-JSON-serializable fields before writing summary.
    ...
    summary = dict(
        ...
        leaf_ote_tuned_own_bias=float(own_tuned),
        leaf_ote_argmax_lb_bias=float(fb_argmax_bal), leaf_ote_errs=fb_errs,
        leaf_ote_pcr=fb_pcr.tolist(), jaccard_vs_4stack=j_vs_4,
        sweep=rows_clean,
    )
    (ART / "leaf_ote_blend_results.json").write_text(json.dumps(summary, indent=2))
