"""Blend LGBM-dist 5-seed bag + routed XGBoost-dist OOF probs.

Mirrors scripts/blend_lgbm_xgb_dist.py but uses the routed-XGB OOF
(XGB trained on dgp_score ∉ {1,2}, rule for those rows at predict
time) instead of plain XGB-dist.

Inputs:
    scripts/artifacts/oof_lgbm_dist_bag.npy
    scripts/artifacts/test_lgbm_dist_bag.npy
    scripts/artifacts/oof_xgb_dist_routed.npy
    scripts/artifacts/test_xgb_dist_routed.npy

Outputs:
    scripts/artifacts/blend_lgbm_xgb_routed_results.json
    submissions/submission_blend_lgbm_xgb_routed.csv (best α)
"""
...
def main() -> None:
    ...
    oof_xgb = np.load(ART / "oof_xgb_dist_routed.npy")
    test_lgbm = np.load(ART / "test_lgbm_dist_bag.npy")
    test_xgb = np.load(ART / "test_xgb_dist_routed.npy")
    print(f"oof_lgbm {oof_lgbm.shape}  oof_xgb_routed {oof_xgb.shape}")

    _, lgbm_bal = tune_log_bias(oof_lgbm, y, prior)
    _, xgb_bal = tune_log_bias(oof_xgb, y, prior)
    print(f"standalone tuned  lgbm_bag={lgbm_bal:.5f}  xgb_routed={xgb_bal:.5f}")
    ...
    print(f"reference non-routed blend (LGBM-bag x XGB) : 0.97327")
    print(f"Δ vs non-routed blend : {winner_tuned - 0.97327:+.5f}")
    ...
    pd.DataFrame({ID: te_ids, TARGET: [IDX2CLS[i] for i in test_pred_idx]}).to_csv(
        SUB / "submission_blend_lgbm_xgb_routed.csv", index=False
    )

    with open(ART / "blend_lgbm_xgb_routed_results.json", "w") as f:
        json.dump({
            "standalone_lgbm_bag_tuned": float(lgbm_bal),
            "standalone_xgb_routed_tuned": float(xgb_bal),
    ...
    print(f"submission written to {SUB/'submission_blend_lgbm_xgb_routed.csv'}")
