"""R11: RF natural with 9-component bank — R10's 8 + 3way_recipe025 (LB 0.98005).

R10 added Tier 1b 4-stack as 8th component → +0.00040 OOF, LB -0.00010
(near-tie). UNIFORM PCR improvement signal was real.

R11 tests COMPOUNDING: add a SECOND LB-validated submission as 9th
component. 3way_recipe025 = log_blend(recipe + pseudo_s1 + pseudo_s7;
0.25/0.35/0.40) with bias [1.4324, 1.4689, 3.4008]. LB 0.98005.

3way_recipe025 is structurally distinct from tier1b 4-stack:
  4-stack:    LB-best 3-stack + xgb_metastack__iso α=0.30 (meta-stacker layer)
  3way:       Pure log-blend of 3 recipe variants (no meta-stacker)

Both LB-validated; both come from different operating points on the
Pareto frontier.

Hypothesis: if R10's +0.00040 OOF lift was due to LB-validated component
diversity, adding ANOTHER LB-validated submission should produce
compounding benefit. Counter-hypothesis: the LB-validated diversity is
saturated at 1 component; 2 is too many.

Outputs (suffix _r11_with_3way):
  oof_sklearn_rf_meta_natural_r11_with_3way.npy
  test_sklearn_rf_meta_natural_r11_with_3way.npy
  sklearn_rf_meta_natural_r11_with_3way_results.json
  submission_sklearn_rf_meta_natural_r11_with_3way_standalone.csv
"""
...
def main():
    ...
    # 9th component: 3way_recipe025 (LB 0.98005)
    log("building 3way recipe blend as 9th component")
    ...
    threeway_o = np.exp(w3[0] * safelog(r[0]) + w3[1] * safelog(s1[0]) + w3[2] * safelog(s7[0]))
    threeway_o = normed(threeway_o / threeway_o.sum(1, keepdims=True))
    threeway_t = np.exp(w3[0] * safelog(r[1]) + w3[1] * safelog(s1[1]) + w3[2] * safelog(s7[1]))
    threeway_t = normed(threeway_t / threeway_t.sum(1, keepdims=True))
    pool["3way_recipe025"] = (threeway_o, threeway_t)
    log(f"  + 3way_recipe025 (LB 0.98005)")
    ...
    log(f"=== R11 OOF argmax = {overall:.5f}  tuned = {tuned:.5f}  bias = {bias.round(4).tolist()}")
    ...
    log("=== vs LB 0.98129 (RF natural 7-component) ===")
    ...
    log("=== vs R10 (8-component with 4-stack only) ===")
    ...
    log(f"  Δ vs R10 errs = {int(errs) - int(errs_r10):+d}")

    test_pred_r11 = (safelog(test_pred) + bias).argmax(1)
    test_pred_lb = (safelog(lb_test) + bias_lb).argmax(1)
    rows_diff = (test_pred_r11 != test_pred_lb).sum()
    net_h = int(((test_pred_r11 == 2) & (test_pred_lb != 2)).sum() -
                ((test_pred_lb == 2) & (test_pred_r11 != 2)).sum())
    churn_h = int(((test_pred_r11 == 2) ^ (test_pred_lb == 2)).sum())
    ...
    sub = pd.DataFrame({"id": test_ids, TARGET: [IDX2CLS[i] for i in test_pred_r11]})
