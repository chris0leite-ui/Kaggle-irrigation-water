"""R4: RF natural meta DROP xgb_dist_digits (only negative-drift component).

LB-best 7-component drift profiles:
  rawashishsin_2600                 drift = [+1.10, +0.80, -0.40]
  recipe_full_te_catboost_natural   drift = [+0.60, +0.80, -0.70]
  recipe_full_te_catboost           drift = [+1.20, +0.90, -0.60]  (R3 dropped)
  recipe_full_te                    drift = [+0.90, +0.50, +0.00]
  realmlp                           drift = [+0.70, +0.50, +0.00]
  xgb_corn                          drift = [+0.00, -0.10, +0.00]
  xgb_dist_digits                   drift = [-0.80, -0.50, +0.00]  ← R4 drop

xgb_dist_digits is the ONLY component with negative drift_L AND
drift_M. Tests "drift-cancellation" hypothesis: if R4 makes the
Pareto trade WORSE than R3, removing the only negative-drift
balancer matters because the meta's natural-cal output is achieved
via averaging-cancellation across mixed drift directions.

R4 LIFTS  → bank tolerates monotone drift; balancing not required.
R4 NULLS  → balancing matters but not significantly.
R4 REGRSS → drift-cancellation hypothesis confirmed.

Architecture frozen identical to LB 0.98129.

Outputs (suffix _r4_drop_digits):
  scripts/artifacts/oof_sklearn_rf_meta_natural_r4_drop_digits.npy
  scripts/artifacts/test_sklearn_rf_meta_natural_r4_drop_digits.npy
  scripts/artifacts/sklearn_rf_meta_natural_r4_drop_digits_results.json
  submissions/submission_sklearn_rf_meta_natural_r4_drop_digits_standalone.csv
"""
...
SUFFIX = "_r4_drop_digits"

# 6-component bank: 7 LB-best DROP xgb_dist_digits (only negative-drift component).
BANK = [
    "rawashishsin_2600",                  # drift = [+1.10, +0.80, -0.40]
    "recipe_full_te_catboost_natural",    # drift = [+0.60, +0.80, -0.70]
    "recipe_full_te_catboost",            # drift = [+1.20, +0.90, -0.60]
    "recipe_full_te",                     # drift = [+0.90, +0.50, +0.00]
    "realmlp",                            # drift = [+0.70, +0.50, +0.00]
    "xgb_corn",                           # drift = [+0.00, -0.10, +0.00]
    # "xgb_dist_digits",                  # R4 DROP: drift = [-0.80, -0.50, +0.00]
]
...
def load_bank(n_tr, n_te):
    log(f"loading R4 6-component drop bank (drop xgb_dist_digits — only -drift component)")
    pool = {}
    for name in BANK:
