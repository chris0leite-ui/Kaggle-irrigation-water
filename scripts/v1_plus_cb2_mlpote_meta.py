"""Retrain v1 meta with recipe_catboost_v2 + recipe_mlp_ote added.

Two new components from this session:
  recipe_catboost_v2 (Kaggle GPU): tuned 0.97921, depth=5, Bayesian, random_strength=2
  recipe_mlp_ote: tuned 0.96958, NN with OTE features (520-d input)

CatBoost-v2 clearly above the 0.97 absorption threshold; MLP-OTE is
borderline but might add as the only NN-base in v1's pool that uses
OTE. Test both as additions.

Pool: v1's 62 + 2 = 64 components. Same SKF(seed=42), same XGB HPs.

Outputs:
  scripts/artifacts/oof_xgb_metastack_v1_plus_cb2mlpote.npy
  scripts/artifacts/test_xgb_metastack_v1_plus_cb2mlpote.npy
  scripts/artifacts/v1_plus_cb2mlpote_meta_results.json
"""
...
NEW_ADDITIONS = ["recipe_catboost_v2", "recipe_mlp_ote"]
V1_PLUS_POOL = V1_FULL_POOL + NEW_ADDITIONS


def log(m): print(f"[{time.strftime('%H:%M:%S')}] PCM: {m}", flush=True)
...
def main():
    ...
    log(f"v1 + cb2 + mlp_ote pool: {len(V1_PLUS_POOL)} components ({len(V1_FULL_POOL)} + {len(NEW_ADDITIONS)} new)")
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X_tr, y)):
        ckpt_oof  = ART / f"_v1pcm_fold{fold}_oof.npy"
        ckpt_test = ART / f"_v1pcm_fold{fold}_test.npy"
        ckpt_meta = ART / f"_v1pcm_fold{fold}_meta.json"
    ...
    np.save(ART / "oof_xgb_metastack_v1_plus_cb2mlpote.npy", oof_meta)
    np.save(ART / "test_xgb_metastack_v1_plus_cb2mlpote.npy", test_meta)
    ...
    log(f"\n  primary OOF (4-stack + v1+cb2+mlp_ote_iso @α=0.30) = {bal(prim_o, y):.5f}")
    ...
    json_path = ART / "v1_plus_cb2mlpote_meta_results.json"
