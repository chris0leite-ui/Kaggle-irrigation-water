"""XGBoost on the distance-to-threshold feature set + pairwise rule×non-rule FE.

Same feature pack as scripts/seed_bag_dist_fe.py (51 features):
  distance + DGP score + 8 pairwise interactions targeting the
  non-rule features with d>0.03 on flipped rows (humidity,
  prev_irrigation, EC, field_area).

Pipeline mirrors benchmark_xgb_dist.py: 5-fold stratified, XGB
multi:softprob level-wise hist, early stopping, log-bias coord-ascent.

Artefacts:
  scripts/artifacts/oof_xgb_dist_fe.npy
  scripts/artifacts/test_xgb_dist_fe.npy
  scripts/artifacts/xgb_dist_fe_results.json
  submissions/submission_xgb_dist_fe_tuned.csv
"""
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # rule × non-rule pairwise FE (matches seed_bag_dist_fe.py)
...
def main() -> None:
    ...
    log("running 5-fold stratified XGBoost")
    ...
    print("\n=== XGBoost + distance + pairwise FE (OOF bal_acc) ===")
    print(f"  argmax               : {argmax_bal:.5f}")
    print(f"  prior-reweight       : {reweight_bal:.5f}")
    print(f"  tuned log-bias       : {tuned_bal:.5f}")

    np.save(ART_DIR / "oof_xgb_dist_fe.npy", oof)
    np.save(ART_DIR / "test_xgb_dist_fe.npy", test_pred)
    with open(ART_DIR / "xgb_dist_fe_results.json", "w") as f:
        json.dump({
            "seed": SEED,
            "n_folds": N_FOLDS,
    ...
    pd.DataFrame({ID: te[ID], TARGET: [IDX2CLS[i] for i in tuned_test_idx]}).to_csv(
        OUT_DIR / "submission_xgb_dist_fe_tuned.csv", index=False
    )
