...
def run_cv(train: pd.DataFrame, test: pd.DataFrame, info: dict) -> dict:
    ...
    # SMOTE-NC needs column indices. All cats + combos + digits + num_as_cat
    # + tres are integer-coded after load_and_engineer; numerics are floats.
    # For SMOTE-NC, mark integer-coded "categorical" columns explicitly.
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(train, y), 1):
        ...
        # Apply SMOTE-NC to oversample High class before OTE fit.
        t0 = time.time()
        ...
        # OrderedTE on augmented train only; apply to val+test.
        t0 = time.time()
