"""R1: RF natural meta on 8-component expanded bank.

LB-best (LB 0.98129) = 7-component bank. R1 adds ONE component:
recipe_full_te_catboost_skte (Pick 2b CB sklearn-TE; bias_H=2.80, NOT
natural-cal). Tests whether a non-natural-cal new component still
contributes to a natural-cal RF meta via diversity.

Architecture frozen identical to LB 0.98129 (n_est=500, max_depth=12,
class_weight=None, bootstrap=True, seed=42). Single seed since A2 closed
seed-bagging.

Outputs (suffix _r1_8comp):
  scripts/artifacts/oof_sklearn_rf_meta_natural_r1_8comp.npy
  scripts/artifacts/test_sklearn_rf_meta_natural_r1_8comp.npy
  scripts/artifacts/sklearn_rf_meta_natural_r1_8comp_results.json
  submissions/submission_sklearn_rf_meta_natural_r1_8comp_standalone.csv
    (NOT auto-LB-probed; await user approval per CLAUDE.md rule)
"""
...
# 7 LB-best + cb_skte (single new addition for R1 isolation test)
BANK = [
    "rawashishsin_2600",
    "recipe_full_te_catboost_natural",
    "recipe_full_te_catboost_skte",       # R1 addition (bias_H=2.80, non-natural-cal)
...
def load_bank(n_tr, n_te):
    log(f"loading R1 8-component bank")
    pool = {}
    for name in BANK:
