...
from features import NUMS, TARGET, SEED, IDX2CLS
...
def run_cv(train: pd.DataFrame, test: pd.DataFrame, orig: pd.DataFrame,
    ...
    for fold, (tr_idx, va_idx) in enumerate(
        skf.split(train[NUMS], y), 1,
    ):
        print(f"=== fold {fold}/{n_folds} ===", flush=True)
        fold_tr = pd.concat([train.iloc[tr_idx], orig], axis=0,
                            ignore_index=True)
        fold_va = train.iloc[va_idx].copy()
        tr_ds = make_dataset(fold_tr, NUMS, target=TARGET)
        va_ds = make_dataset(fold_va, NUMS, target=TARGET)
        te_ds = make_dataset(test.assign(**{TARGET: 0}), NUMS,
                             target=TARGET)
...
def save_outputs(out_dir: Path, oof, test_pred, test_ids, fold_ba,
    ...
    np.save(out_dir / f"oof_excelformer{suffix}.npy", oof)
    np.save(out_dir / f"test_excelformer{suffix}.npy", test_pred)
    pred = (np.log(np.clip(test_pred, 1e-9, 1.0)) + bias).argmax(axis=1)
    sub = pd.DataFrame({
        "id": test_ids, TARGET: [IDX2CLS[i] for i in pred],
    })
    sub.to_csv(out_dir / f"submission_excelformer{suffix}_tuned.csv", index=False)
    (out_dir / f"excelformer{suffix}_results.json").write_text(json.dumps({
    ...
    print(f"wrote oof_excelformer{suffix}.npy / test_excelformer{suffix}.npy / "
          f"submission_excelformer{suffix}_tuned.csv / "
          f"excelformer{suffix}_results.json", flush=True)
