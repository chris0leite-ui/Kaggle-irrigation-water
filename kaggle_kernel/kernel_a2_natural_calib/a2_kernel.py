#!/usr/bin/env python3
"""
A2: Natural-calibration hybrid base (rawashishsin v3 + our recipe FE additions)
================================================================================
Builds on rawashishsin's pipeline (LB 0.98109) by adding three feature blocks
from our recipe pipeline:

  A2 additions over rawashishsin v3:
    - 8 signed/abs continuous distance-to-threshold features
      (a2_sm_dist, a2_rf_dist, a2_tc_dist, a2_ws_dist + their abs)
    - 16 ORIG mean/std per cat from 10k anchor (a2_ORIG_*)
    - 3 LR-formula logits per row (a2_logit_pL/pM/pH)

  Goal: standalone OOF ≥ 0.98016 (their CV) AND bias on High near 0
        AND LB transfer ≥ 0.98109 (their LB).

  If A2 produces:
    - Raw High bias near 0: natural calibration mechanism preserved
    - Standalone OOF higher than rawashishsin: FE additions added signal
    - LB transfer ≥ 0.98109: transfers AND beats baseline
  Then we have an own-work differentiator above the public-kernel copy.

Inherits rawashishsin's mechanism (verified cell-by-cell from public ipynb):
...
Saves: oof_a2_natural_calib.npy, test_a2_natural_calib.npy, results.json,
...
OOF_PATH = OUT / f"oof_a2_natural_calib{SUFFIX}.npy"
TEST_PATH = OUT / f"test_a2_natural_calib{SUFFIX}.npy"
RESULTS_PATH = OUT / f"a2_natural_calib{SUFFIX}_results.json"
...
def add_threshold_distances(df):
    ...
    # A2 additions: signed continuous distance to each rule threshold.
    # Captures within-cell flip likelihood (HOW FAR a row is from each threshold).
    # NOT in rawashishsin's pipeline. Critical own-recipe insight per CLAUDE.md
    # 2026-04-21 entry "DGP features (score + distance-to-threshold) DO help LGBM".
    df["a2_sm_dist"]   = df["Soil_Moisture"] - 25
    df["a2_rf_dist"]   = df["Rainfall_mm"] - 300
    df["a2_tc_dist"]   = df["Temperature_C"] - 30
    df["a2_ws_dist"]   = df["Wind_Speed_kmh"] - 10
    df["a2_sm_abs"]    = df["a2_sm_dist"].abs()
    df["a2_rf_abs"]    = df["a2_rf_dist"].abs()
    df["a2_tc_abs"]    = df["a2_tc_dist"].abs()
    df["a2_ws_abs"]    = df["a2_ws_dist"].abs()
    return df
...
def main():
    ...
    # A2: ORIG mean/std per cat — leak-free aggregates from 10k anchor.
    # Mirrors recipe_features.add_orig_mean_std. Cats only (numerics already
    # in NUM_CAT_AGG in their pipeline).
    A2_ORIG_STATS = []
    print("[a2-fe] adding ORIG mean/std per cat from 10k anchor...", flush=True)
    for c in CAT_COLS:
        agg = orig_fe.groupby(c, observed=True)[TARGET].agg(['mean', 'std']).reset_index()
        agg.columns = [c, f"a2_ORIG_{c}_mean", f"a2_ORIG_{c}_std"]
        for df in (train_fe, test_fe, orig_fe):
            merged = df[[c]].astype('object').merge(agg.astype({c: 'object'}), on=c, how='left')
            df[f"a2_ORIG_{c}_mean"] = merged[f"a2_ORIG_{c}_mean"].fillna(0.5).astype(np.float32).values
            df[f"a2_ORIG_{c}_std"]  = merged[f"a2_ORIG_{c}_std"].fillna(0).astype(np.float32).values
        A2_ORIG_STATS.extend([f"a2_ORIG_{c}_mean", f"a2_ORIG_{c}_std"])
    print(f"[a2-fe] +{len(A2_ORIG_STATS)} ORIG stat cols", flush=True)

    # A2: LR-formula logits (cdeotte's exact 9-binary linear formula → P_class).
    # 3 numeric cols (logit_P_low, logit_P_med, logit_P_high). Mirrors
    # recipe_features.add_lr_formula_logits.
    LR_COEF = {
        # cdeotte's published coefficients (from CLAUDE.md 2026-04-21 entry)
        "soil_lt_25":   ( 2.0,  -2.0),
        "rain_lt_300":  ( 2.0,  -2.0),
        "temp_gt_30":   ( 1.0,  -1.0),
        "wind_gt_10":   ( 1.0,  -1.0),
        "Mulching_Used_Yes":    (-1.0,  1.0),
        "Crop_Growth_Stage_Flowering":   ( 0.5, -0.5),
        "Crop_Growth_Stage_Vegetative":  ( 0.5, -0.5),
        "Crop_Growth_Stage_Sowing":      (-0.5, 0.5),
        "Crop_Growth_Stage_Harvest":     (-0.5, 0.5),
    }
    print("[a2-fe] adding LR-formula logits (3 numeric cols)...", flush=True)
    A2_LR_LOGITS = ["a2_logit_pL", "a2_logit_pM", "a2_logit_pH"]
    for df in (train_fe, test_fe, orig_fe):
        # 5 binary indicators + 4 stage one-hots
        mu_yes = (df["Mulching_Used"].astype(str) == "Yes").astype(np.float32).values
        stage_str = df["Crop_Growth_Stage"].astype(str).values
        flow = (stage_str == "Flowering").astype(np.float32)
        vege = (stage_str == "Vegetative").astype(np.float32)
        sow  = (stage_str == "Sowing").astype(np.float32)
        harv = (stage_str == "Harvest").astype(np.float32)
        # Linear combination: high_score - low_score per cdeotte's formula
        lin = (
            2.0 * df["soil_lt_25"].astype(np.float32).values +
            2.0 * df["rain_lt_300"].astype(np.float32).values +
            1.0 * df["temp_gt_30"].astype(np.float32).values +
            1.0 * df["wind_gt_10"].astype(np.float32).values +
           -1.0 * mu_yes +
            0.5 * flow + 0.5 * vege - 0.5 * sow - 0.5 * harv
        )
        # Map linear score → 3 logits via softmax-like transform: (-lin, 0, +lin) scaled.
        df["a2_logit_pL"] = (-lin).astype(np.float32)
        df["a2_logit_pM"] = (0.5 * np.abs(lin - 3.0) - 1.5).astype(np.float32)
        df["a2_logit_pH"] = (lin - 3.0).astype(np.float32)
    print(f"[a2-fe] +{len(A2_LR_LOGITS)} LR-logit cols", flush=True)
    print(f"[fe] NGRAM={len(NGRAM)} BIN_CAT_INT={len(BIN_CAT_INT)} NUM_CAT_AGG={len(NUM_CAT_AGG)} "
          ...
          f"A2_ORIG={len(A2_ORIG_STATS)} A2_LR={len(A2_LR_LOGITS)}",
          flush=True)
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(fold_splits):
        ...
        if elapsed >= WALL_KILL_SEC:
            ...
            break
        ckpt_oof = OUT / f"oof_a2_natural_calib{SUFFIX}_fold{fold_idx}.npy"
        ckpt_test = OUT / f"test_a2_natural_calib{SUFFIX}_fold{fold_idx}.npy"
    ...
    n_completed = sum(1 for i in range(len(fold_splits))
                      if (OUT / f"oof_a2_natural_calib{SUFFIX}_fold{i}.npy").exists())
