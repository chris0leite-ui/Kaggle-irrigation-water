...
Outputs (written to /kaggle/working/):
  oof_catboost_recipe_gpu.npy
  test_catboost_recipe_gpu.npy
  catboost_recipe_gpu_results.json
  submission_catboost_recipe_gpu_tuned.csv
"""
...
def run_cv(train, test, info, a_ote=1.0):
    ...
    cb_params = dict(
        iterations=1500 if SMOKE else 16000,
        depth=4,
        learning_rate=0.1,
        l2_leaf_reg=10.0,
        loss_function="MultiClass",
...
def main():
    ...
    np.save(OUT_DIR / "oof_catboost_recipe_gpu.npy", result["oof"])
    np.save(OUT_DIR / "test_catboost_recipe_gpu.npy", result["test"])
    ...
    sub.to_csv(OUT_DIR / "submission_catboost_recipe_gpu_tuned.csv", index=False)
    log("wrote submission_catboost_recipe_gpu_tuned.csv")
    ...
    summary = dict(
        ...
        cb_params_iterations=(1500 if SMOKE else 16000),
        cb_params_depth=4,
        cb_params_l2_leaf_reg=10.0,
        cb_params_od_wait=(100 if SMOKE else 500),
    )
    with open(OUT_DIR / "catboost_recipe_gpu_results.json", "w") as f:
        json.dump(summary, f, indent=2)
    log("wrote catboost_recipe_gpu_results.json")
