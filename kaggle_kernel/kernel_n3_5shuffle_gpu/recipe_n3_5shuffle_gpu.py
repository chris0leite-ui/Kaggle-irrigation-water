"""N3 5-shuffle OTE concat — full 5-fold GPU production.

Companion to kernel_recipe_5shuffle_gpu, which validated GPU XGB speed on
fold 1 (17.3 min wall, 0.52 s/iter — 25x faster than Kaggle CPU).
Projection for full 5-fold: ~95 min total. Hard kill at 4h (~2.5x
safety margin per CLAUDE.md GPU rule override conditions).

Reference: yunsuxiaozi/pss6e4-lgb-advanced-cv-0-97997 — claims
CV 0.97997 standalone with K=5 OTE shuffle concat.

Outputs to /kaggle/working/:
  oof_recipe_5shuffle_gpu.npy
  test_recipe_5shuffle_gpu.npy
  recipe_5shuffle_gpu_results.json (per-fold metrics + tuned bias)
  submission_recipe_5shuffle_gpu.csv (only if all 5 folds completed)

Per-fold checkpoints written incrementally for rehydrate safety.
"""
...
# *** PRODUCTION KNOBS ***
# All 5 folds; 4h hard kill (validation showed ~95min projected, 2.5x margin).
RUN_FOLD_INT = None   # run all folds
TOTAL_KILL_SEC = 4 * 60 * 60   # 4h
...
def run_cv_5shuffle(train, test, info, t_start):
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(train, y), 1):
        # *** Optional fold filter (disabled for production; left for future use) ***
        if RUN_FOLD_INT is not None and fold != RUN_FOLD_INT:
            log(f"skipping fold {fold} (RUN_FOLD_INT={RUN_FOLD_INT})")
            continue
        ...
        np.save(OUT_DIR / f"oof_recipe_5shuffle_gpu_fold{fold}.npy", oof)
        np.save(OUT_DIR / f"test_recipe_5shuffle_gpu_fold{fold}.npy", test_pred)
...
def main():
    ...
    np.save(OUT_DIR / "oof_recipe_5shuffle_gpu.npy", result["oof"])
    np.save(OUT_DIR / "test_recipe_5shuffle_gpu.npy", result["test"])
    log(f"saved oof_recipe_5shuffle_gpu.npy + test_recipe_5shuffle_gpu.npy")
    ...
    if result["folds_completed"] == N_FOLDS:
        ...
        sub_path = OUT_DIR / "submission_recipe_5shuffle_gpu.csv"
    ...
    res_path = OUT_DIR / "recipe_5shuffle_gpu_results.json"
