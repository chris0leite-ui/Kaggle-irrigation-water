...
Outputs in /kaggle/working/:
  oof_realmlp_ens2.npy (n_train, 3)
  test_realmlp_ens2.npy (n_test, 3)
  realmlp_ens2_results.json

Tier 1a n_ens=2 @ FULL n_epochs=40 (2026-04-26). Diagnostic between
n_ens=1 (OOF 0.97636, LB +0.00003 in 3-stack) and n_ens=4 (NULL — OOF
WORSE than n_ens=1 standalone, blend NULL). The n_ens=4 NULL was
attributed in CLAUDE.md to one of two mechanisms:
  (a) under-convergence: n_epochs=25 forced by 1h cap with 4× heads
  (b) variance floor: n_ens=1 already at the per-row variance floor

n_ens=2 @ n_epochs=40 disambiguates these:
  - If it BEATS n_ens=1 standalone (errs lower, OOF tuned higher) →
    (a) is correct; an overnight n_ens=4 @ n_epochs=40 run is justified.
  - If it PLATEAUS at n_ens=1 → (b) is correct, RealMLP variance
    floor is structural, lever closed.

Config (chosen to fit cleanly under 1h GPU cap with full convergence):
  - n_ens=2 (2× BatchEnsemble heads — middle of n_ens=1 vs n_ens=4).
  - n_epochs=40 (FULL, matches n_ens=1's working config).
  - TargetEncoder cv=2 (unchanged from n_ens=1 / n_ens=4).
...
ETA: ~45-55 min on P100 GPU at n_ens=2 / n_epochs=40 / 5 folds. Two
heads at full epochs ≈ 1.6× n_ens=1's wall (n_ens=1 was 38 min, so
this projects to ~60 min — tight against the 55-min kill). If fold-1
runs >12 min, expect a partial 3-4 fold result (kill-saved cleanly).

Fold-1 gate (computed by scripts/blend_realmlp_ens2.py post-pull):
  errs_fold1 < 1.04× anchor_fold1  AND  Jaccard < 0.65
  → if both pass, finish 5-fold + run full blend gate.
...
# the SMOKE kernel has completed successfully (CLAUDE.md SMOKE-first rule).
IS_SMOKE = True   # smoke-first; flip to False for production after green
...
def run_cv(train, test, orig, NUMS, CATS, combo_cols) -> dict:
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(train, y), 1):
        ...
        log(f"  fitting RealMLP_TD on {tr_frame.shape[1]} features, "
            f"{len(tr_frame):,} rows")
        t0 = time.time()
        # n_ens=2 @ n_epochs=40: diagnostic between n_ens=1 (worked,
        # LB +0.00003 in 3-stack) and n_ens=4 (nulled @ n_epochs=25).
        # If n_ens=2 @ full epochs beats n_ens=1, the n_ens=4 NULL was
        # under-convergence; if it plateaus, n_ens=1 already at variance
        # floor and the lever closes here.
        # n_epochs=40 (matches n_ens=1's working config — full
        # convergence, not under-converged like n_ens=4 was at 25).
        model = RealMLP_TD_Classifier(
            n_cv=1, n_refit=0, n_ens=2,
            device="cuda",
...
def main():
    ...
    np.save(OUT_DIR / "oof_realmlp_ens2.npy", oof)
    np.save(OUT_DIR / "test_realmlp_ens2.npy", test_pred)
    log(f"wrote oof_realmlp_ens2.npy + test_realmlp_ens2.npy")

    pred = (np.log(np.clip(test_pred, 1e-9, 1.0)) + bias).argmax(axis=1)
    sub = pd.DataFrame({"id": test_ids,
                        TARGET: [IDX2CLS[i] for i in pred]})
    sub_path = OUT_DIR / "submission_realmlp_ens2_tuned.csv"
    ...
    (OUT_DIR / "realmlp_ens2_results.json").write_text(json.dumps(json_out, indent=2))
    log(f"wrote realmlp_ens2_results.json")
