"""N3 5-shuffle OTE concat — yunsuxiaozi training-augmentation recipe.

Mechanism: per fold, fit OrderedTE with K=5 different shuffle seeds, concat
the K resulting DataFrames as augmented training rows (5x train pool). The
model sees the same (row, key) categorical assignment with 5 DIFFERENT TE
values — turns OTE estimation noise into a data-augmentation signal,
structurally distinct from prior K-shuffle averaging that smooths noise away.

Reference: yunsuxiaozi/pss6e4-lgb-advanced-cv-0-97997 (kernel audit round 4).
Their CV 0.97997 standalone → suggests this lever materially moves a
single-model OOF.

Smoke evidence (local): +0.00177 OOF lift on 20k subset over recipe smoke
baseline. Production blocked locally on container rehydrate; this Kaggle
kernel runs on the 9h cap with no rehydrate risk.

Inlined from:
  scripts/common.py (tune_log_bias, fast_bal_acc)
  scripts/recipe_features.py (FE blocks)
  scripts/recipe_ote.py (OrderedTE)
  scripts/recipe_ote_5shuffle.py (fit_concat_5shuffle)
  scripts/recipe_full_te.py (load_and_engineer)
  scripts/recipe_full_te_5shuffle.py (run_cv_5shuffle)

Outputs (to /kaggle/working/):
  oof_recipe_5shuffle.npy
  test_recipe_5shuffle.npy
  recipe_5shuffle_results.json
  submission_recipe_5shuffle.csv (only if all 5 folds completed)

Wall-time budget: 8h hard kill (Kaggle 9h cap).

SMOKE=1 → 20k train, 2 folds, K=2 shuffle, fewer XGB iters (~3 min).
"""
...
# Wall-time hard kill
TOTAL_KILL_SEC = 8 * 60 * 60   # 8h, leaves 1h headroom under Kaggle's 9h cap
...
def run_cv_5shuffle(train, test, info, t_start):
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(train, y), 1):
        ...
        np.save(OUT_DIR / f"oof_recipe_5shuffle_fold{fold}.npy", oof)
        np.save(OUT_DIR / f"test_recipe_5shuffle_fold{fold}.npy", test_pred)
...
def main():
    ...
    np.save(OUT_DIR / "oof_recipe_5shuffle.npy", result["oof"])
    np.save(OUT_DIR / "test_recipe_5shuffle.npy", result["test"])
    log(f"saved oof_recipe_5shuffle.npy + test_recipe_5shuffle.npy")
    ...
    if result["folds_completed"] == N_FOLDS:
        ...
        sub_path = OUT_DIR / "submission_recipe_5shuffle.csv"
    ...
    res_path = OUT_DIR / "recipe_5shuffle_results.json"
