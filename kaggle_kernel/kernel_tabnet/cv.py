...
def save_outputs(out_dir: Path, oof, test_pred, test_ids, fold_ba,
    ...
    np.save(out_dir / f"oof_tabnet{suffix}.npy", oof)
    np.save(out_dir / f"test_tabnet{suffix}.npy", test_pred)
    pred = (np.log(np.clip(test_pred, 1e-9, 1.0)) + bias).argmax(axis=1)
    sub = pd.DataFrame({
        "id": test_ids, TARGET: [IDX2CLS[i] for i in pred],
    })
    sub.to_csv(out_dir / f"submission_tabnet{suffix}_tuned.csv", index=False)
    (out_dir / f"tabnet{suffix}_results.json").write_text(json.dumps({
    ...
    print(f"wrote oof_tabnet{suffix}.npy / test_tabnet{suffix}.npy / "
          f"submission_tabnet{suffix}_tuned.csv / "
          f"tabnet{suffix}_results.json", flush=True)
