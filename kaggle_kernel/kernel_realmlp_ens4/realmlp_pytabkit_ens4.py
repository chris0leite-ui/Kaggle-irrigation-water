...
Outputs in /kaggle/working/:
  oof_realmlp_ens4.npy (n_train, 3)
  test_realmlp_ens4.npy (n_test, 3)
  realmlp_ens4_results.json

Tier 1a n_ens=4 retry (2026-04-24). Builds on the n_ens=1 kernel
(OOF tuned 0.97636, Jaccard 0.62 vs LB-best 3-way but magnitude-
trapped in the hand-picked α=0.375 blend, LB 0.97991). The 3-stack
follow-up (LB 3-way + realmlp@0.2 + nonrule_iso@0.075) landed LB
0.98008 (+0.00003 over prior best), validating RealMLP as a legit
blend leg. Ensembling to n_ens=4 should drop per-row variance ~2×
and produce errs that are safely below anchor, unlocking another
+0.0002-0.0003 LB via the same 3-stack mechanism.

Config (adjusted for n_ens=4 within the 1h Kaggle-GPU cap):
  - n_ens=4 (4× BatchEnsemble heads sharing most weights via einsum).
  - n_epochs=25 (shorter than n_ens=1's 40 — the shared-core does
    most of the work, so per-head convergence time is similar).
  - TargetEncoder cv=2 (unchanged from n_ens=1).
...
ETA: 40–55 min on P100 GPU at n_ens=4 / n_epochs=25 / 5 folds.
...
def run_cv(train, test, orig, NUMS, CATS, combo_cols) -> dict:
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(train, y), 1):
        ...
        log(f"  fitting RealMLP_TD on {tr_frame.shape[1]} features, "
            f"{len(tr_frame):,} rows")
        t0 = time.time()
        # n_ens=4: 4 parallel BatchEnsemble heads sharing most weights
        # via einsum. Tier 1a retry — the n_ens=1 kernel hit OOF tuned
        # 0.97636 but blended into the current 3-stack at only +0.00003
        # LB due to magnitude trap (errs +358 vs anchor). Ensembling
        # should cut per-row variance roughly 2×, plausibly dropping
        # errs below anchor and unlocking a +0.0002-0.0003 LB lift via
        # the same 3-stack mechanism.
        # n_epochs=25 (not 40) — tighter to stay under the 1h cap with
        # 4× internal heads. Shared-weight core does most of the work,
        # so per-head convergence time is similar to n_ens=1.
        model = RealMLP_TD_Classifier(
            n_cv=1, n_refit=0, n_ens=4,
            device="cuda",
...
def main():
    ...
    np.save(OUT_DIR / "oof_realmlp_ens4.npy", oof)
    np.save(OUT_DIR / "test_realmlp_ens4.npy", test_pred)
    log(f"wrote oof_realmlp_ens4.npy + test_realmlp_ens4.npy")

    pred = (np.log(np.clip(test_pred, 1e-9, 1.0)) + bias).argmax(axis=1)
    sub = pd.DataFrame({"id": test_ids,
                        TARGET: [IDX2CLS[i] for i in pred]})
    sub_path = OUT_DIR / "submission_realmlp_ens4_tuned.csv"
    ...
    (OUT_DIR / "realmlp_ens4_results.json").write_text(json.dumps(json_out, indent=2))
    log(f"wrote realmlp_ens4_results.json")
