...
SUFFIX = "_pseudo" + ("_smoke" if IS_SMOKE else ("_probe" if IS_PROBE else ""))
...
PSEUDO_ROW_WEIGHT = float(os.environ.get("PSEUDO_ROW_WEIGHT", "0.5"))
...
def find_data():
    candidates = {
        ...
        "pseudo": [
            "/kaggle/input/irrigation-t4-pseudo-labels/t4_pseudo_labels.csv",
            "/kaggle/input/t4-pseudo-labels/t4_pseudo_labels.csv",
            "data/t4_pseudo_labels.csv",
        ],
    }
    paths = {}
...
def load_data(paths):
    ...
    pseudo = pd.read_csv(paths["pseudo"])
    print(f"[load] train={train.shape} test={test.shape} orig={orig.shape} pseudo={pseudo.shape}", flush=True)
    return train, test, orig, pseudo


def merge_pseudo_to_test(test, pseudo):
    """Inner join test on id with pseudo-labels.  Returns (test_pseudo_X, y_pseudo)
    where test_pseudo_X is a copy of the matching test rows including their
    original features.  pseudo_label column added as Irrigation_Need."""
    keep = test.merge(pseudo[["id", "Irrigation_Need_pseudo"]], on="id", how="inner")
    keep = keep.rename(columns={"Irrigation_Need_pseudo": TARGET})
    print(f"[pseudo] matched {len(keep)} pseudo rows on id", flush=True)
    return keep
...
def main():
    ...
    paths = find_data()
    train, test, orig, pseudo = load_data(paths)

    # Build pseudo-augmented "synthetic test rows" with v1 RF natural argmax labels
    pseudo_full = merge_pseudo_to_test(test, pseudo)
    print(f"[pseudo] class dist: {pseudo_full[TARGET].value_counts().to_dict()}", flush=True)

    # Map target
    train[TARGET] = train[TARGET].map(TARGET_MAP)
    orig[TARGET] = orig[TARGET].map(TARGET_MAP)
    pseudo_full[TARGET] = pseudo_full[TARGET].map(TARGET_MAP)
    ...
    test_fe = test.copy()
    orig_fe = orig.copy()
    pseudo_fe = pseudo_full.copy()
    train_fe = add_threshold_distances(train_fe)
    test_fe = add_threshold_distances(test_fe)
    orig_fe = add_threshold_distances(orig_fe)
    pseudo_fe = add_threshold_distances(pseudo_fe)
    train_fe = add_formula_features(train_fe)
    test_fe = add_formula_features(test_fe)
    orig_fe = add_formula_features(orig_fe)
    pseudo_fe = add_formula_features(pseudo_fe)
    # NB: ngrams / bin_cat / num_cat_agg / pairs are factorize-based.
    # We need pseudo to share the SAME factorization as train+test+orig.
    # The simplest approach: fit factorize on train+test+orig (as base does)
    # and then map pseudo using its (test_id) row index in the original test_fe.
    ...
    # Map pseudo's id-aligned rows from test_fe (already FE'd).
    # In SMOKE mode test was subsampled, so intersect pseudo ids with FE'd test.
    test_fe_indexed = test_fe.set_index("id")
    pseudo_ids = pseudo_full["id"].values
    keep_mask = np.isin(pseudo_ids, test_fe_indexed.index.values)
    pseudo_kept = pseudo_full.loc[keep_mask].reset_index(drop=True)
    pseudo_fe_complete = test_fe_indexed.loc[pseudo_kept["id"].values].reset_index()
    pseudo_fe_complete[TARGET] = pseudo_kept[TARGET].values
    print(f"[pseudo] FE'd pseudo shape: {pseudo_fe_complete.shape} "
          f"(intersected from {len(pseudo_ids)})", flush=True)

    # cat dtype
    for df in [train_fe, test_fe, orig_fe, pseudo_fe_complete]:
        for col in CAT_COLS:
    ...
    X_pseudo = pseudo_fe_complete.drop([TARGET, "id"], axis=1)
    y_pseudo = pseudo_fe_complete[TARGET].values
    print(f"[shape] X_train={X_train.shape} X_orig={X_orig.shape} X_test={X_test.shape} X_pseudo={X_pseudo.shape}", flush=True)
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(fold_splits):
        ...
        # T4: concat synth_train + orig + pseudo (test rows with v1 RF natural pseudo-labels)
        X_tr_combined = pd.concat([X_tr_real, X_orig, X_pseudo], axis=0).reset_index(drop=True)
        y_tr_combined = np.concatenate([y_tr_real, y_orig, y_pseudo])
        n_real = len(tr_idx)
        n_orig = len(X_orig)
        n_pseudo = len(X_pseudo)
        print(f"[fold {fold_idx}] tr_combined={X_tr_combined.shape} "
              f"(real={n_real} orig={n_orig} pseudo={n_pseudo})", flush=True)
        ...
        # Sample weights: balanced + orig downweight + pseudo downweight
        sample_weights = compute_sample_weight('balanced', y_tr_combined).astype(np.float32)
        # rows in order: [real_train (0..n_real), orig (n_real..n_real+n_orig),
        #                 pseudo (n_real+n_orig..end)]
        sample_weights[n_real:n_real + n_orig] *= ORIG_ROW_WEIGHT
        sample_weights[n_real + n_orig:] *= PSEUDO_ROW_WEIGHT
        print(f"[fold {fold_idx}] mean_w_real={sample_weights[:n_real].mean():.4f} "
              f"mean_w_orig={sample_weights[n_real:n_real+n_orig].mean():.4f} "
              f"mean_w_pseudo={sample_weights[n_real+n_orig:].mean():.4f}", flush=True)
