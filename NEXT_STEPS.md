...
| `claude/improve-predictions-with-dgp-d0ADN` | **W8 EXTRA_FE='w8' on FULL recipe** | ✅ COMPLETE — **NULL (5th FE-on-recipe null)**. Production 5-fold seed=42 ran 2h08min (heavier than expected, prior W1 left CPU residual). Standalone tuned **0.97955** (recipe 0.97967, Δ=−0.00013). Per-fold deltas: +0.00003, −0.00110, −0.00033, +0.00031, −0.00036 (mean −0.00029, well within fold-std noise 0.00067). Blend vs LB-best 4-stack at fixed recipe bias: peak α=0.025 → Δ=**+0.00000** (literally zero). Jaccard vs recipe 0.8671 / vs LB-best 0.8296 — both above 0.80 redundancy threshold. **No submission emitted.** Same A4 (−0.00012) / GBY (−0.00008) pattern. The recipe's 117 OTE features absorb any reconstructable transform of raw features regardless of FE class (cross-products, per-score z-scores, threshold flags, ratios). LB best unchanged at **0.98094**. |
